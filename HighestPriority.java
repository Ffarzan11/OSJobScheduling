...
public class HighestPriority {
    ...
    protected PriorityQueue<JobObject> ready = new PriorityQueue<>(new HighestPriorityComparator());
     ...
     */
    public HighestPriority(JobObject[] jobs) {
        JobObject[] jobsHighestPriority = new JobObject[jobs.length];
        for(int i = 0; i < jobs.length; i++) {
            jobsHighestPriority[i] = new JobObject(jobs[i].arrTime, jobs[i].cpuBurst, jobs[i].priority, jobs[i].remainingTime);
        }
        this.jobs.addAll(Arrays.asList(jobsHighestPriority));
    }
    ...
    public void print() {
        System.out.println("Highest Priority Scheduling");
        System.out.println("| Arrival time\t| CPU Burst\t| Exit\t| Turn Around Time\t| Priority\t|");
